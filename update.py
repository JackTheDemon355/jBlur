...
# --- ADVANCED ERROR WINDOW (REPLACES TINY POPUPS) ---
...
class MegaAnonymizerPro:
    def __init__(self, root):
        ...
        self.active_tool = "SELECT"  # SELECT or PAN
    ...
    def _build_interface(self):
        ...
        # Offline Banner
        self.offline_banner = tk.Label(self.main_box, text="⚠️ OFFLINE MODE ACTIVE — Cloud sync & auto-updates disabled", bg="#D32F2F", fg="white", font=("Segoe UI", 9, "bold"), pady=4)
        ...
        # Support Desk Launcher
        tk.Button(self.ribbon, text="❓ Support & Error Lookup", command=self.open_support_center, bg="#4A148C", fg="white", font=("Segoe UI", 9, "bold"), relief=tk.FLAT).pack(side=tk.RIGHT, padx=10)

        # Body Split Pane
        self.body_frame = tk.Frame(self.main_box, bg="#181818")
        self.body_frame.pack(fill=tk.BOTH, expand=True)

        # --- LEFT SIDEBAR (Controls & Controls Notebook) ---
    ...
    def _set_online_status(self, online):
        if online != self.is_online:
            ...
            if not online:
                self.offline_banner.pack(side=tk.TOP, fill=tk.X, before=self.ribbon)
            else:
    ...
    def update_canvas(self):
        ...
        # Optional Grid / Rule of Thirds Overlays
        if self.show_grid:
    ...
    # Image Adjustments & Transformations
